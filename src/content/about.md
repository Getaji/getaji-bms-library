...
2026/06/18 更新: 表記を絵文字形式に変更しました。

| 確度 | 説明 |
| -- | -- |
|🟢|高/参考にした難易度情報が多く、分散が小さい|
|🟡|中/低と高どちらでもない|
|🔴|低/情報が少なすぎる・分散が大きすぎる|
|❓️|？/情報がない、または皿譜面などの難易度推定を行わない譜面|
...
- "推定sl10" のように書かれている譜面は、推定難易度に幅がある場合、その範囲内で今のところ妥当だと思われる難易度を設定していることを表します。
  - （幅がなくても書いていることもあります）
- 高TOTAL/低TOTAL表記の横に%表記があるものは、beatorajaで算出されるデフォルトTOTAL値との比率を示したものです。以下の計算式で算出されます。
  - `既定値 = Math.max(260.0, (7.605 * notes) / (0.01 * notes + 6.5))`
  - `比率 = TOTAL値 / 既定値`
  - 2026/01/17以降、ページのコメント欄にT/N比（TOTAL値をノーツで割ったもの）と合わせて併記しています。従来コメントに記載していた比率表記は次回以降の更新で削除する予定です。

# お問い合わせ

[Twitter](https://twitter.com/Getaji)のDMか[GitHubのIssue](https://github.com/Getaji/getaji-bms-library/issues)までお願いします。ただしTwitterのDMは反応できない可能性があります。Discordのアカウントをご存じの方はそちらでも構いません。

主に以下のようなお問い合わせを受け付けています。

- 不具合・記載ミスの報告
  - 見逃しているミスがおそらく多くあります。報告いただけるととても助かります。
- 譜面の掲載取り下げ要請
